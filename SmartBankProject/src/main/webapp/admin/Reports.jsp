<%@ page language="java" contentType="text/html; charset=UTF-8"%>
<%@ page import="java.sql.*, java.util.*" %>
<%@ include file="../dbs.jsp" %>
<%
    // Admin access check
    String role = (String) session.getAttribute("role");
    if(role == null || !"admin".equals(role)){
        response.sendRedirect("../Login.jsp?msg=unauthorized");
        return;
    }

    // Daily Metrics
    double dailyDeposits = 0, dailyWithdrawals = 0, dailyTransactions = 0;
    double totalBankBalance = 0;
    int activeAccountsToday = 0;

    List<Map<String,Object>> todayTransactions = new ArrayList<>();
    Connection con = null;
    Statement stmt = null;
    ResultSet rs = null;

    try {
        con = getConnection();
        stmt = con.createStatement();

        // Total Bank Balance
        rs = stmt.executeQuery("SELECT SUM(Balance) AS balanceSum FROM accounts");
        if(rs.next()) totalBankBalance = rs.getDouble("balanceSum");
        rs.close();

        // Active Accounts Today (Accounts with any transaction today)
        rs = stmt.executeQuery("SELECT COUNT(DISTINCT AccountNumber) AS count FROM transactions WHERE DATE(Timestamp) = CURDATE()");
        if(rs.next()) activeAccountsToday = rs.getInt("count");
        rs.close();

        // Daily Deposits & Withdrawals
        rs = stmt.executeQuery(
            "SELECT " +
            "SUM(CASE WHEN Type='Deposit' THEN Amount ELSE 0 END) AS depositSum, " +
            "SUM(CASE WHEN Type='Withdrawal' THEN Amount ELSE 0 END) AS withdrawalSum, " +
            "COUNT(*) AS txnCount " +
            "FROM transactions " +
            "WHERE DATE(Timestamp) = CURDATE()"
        );
        if(rs.next()){
            dailyDeposits = rs.getDouble("depositSum");
            dailyWithdrawals = rs.getDouble("withdrawalSum");
            dailyTransactions = rs.getInt("txnCount");
        }
        rs.close();

        // Today's Transactions
        rs = stmt.executeQuery("SELECT * FROM transactions WHERE DATE(Timestamp) = CURDATE() ORDER BY Timestamp DESC");
        while(rs.next()){
            Map<String,Object> tx = new HashMap<>();
            tx.put("id", rs.getInt("TransactionID"));
            tx.put("accNo", rs.getString("AccountNumber"));
            tx.put("type", rs.getString("Type"));
            tx.put("recipient", rs.getString("RecipientAccount"));
            tx.put("amount", rs.getDouble("Amount"));
            tx.put("desc", rs.getString("Description"));
            tx.put("time", rs.getTimestamp("Timestamp"));
            todayTransactions.add(tx);
        }
        rs.close();

    } catch(Exception e){ e.printStackTrace(); }
    finally { try { if(stmt!=null) stmt.close(); } catch(Exception e){} try { if(con!=null) con.close(); } catch(Exception e){} }
%>

<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Daily Bank Report</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css" rel="stylesheet">
    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    <style>
        body { margin:0; padding:0; display:flex; font-family:'Segoe UI', sans-serif; background:#f8f9fa; }
        .sidebar { width:220px; height:100vh; background:#343a40; color:white; position:fixed; top:0; left:0; overflow-y:auto; padding-top:20px; }
        .sidebar h4 { text-align:center; margin-bottom:20px; font-weight:bold; }
        .sidebar .nav-link { color:#adb5bd; padding:12px 20px; transition:0.3s; display:flex; align-items:center; }
        .sidebar .nav-link:hover, .sidebar .nav-link.active { background:#495057; color:white; }
        .sidebar .nav-link i { margin-right:10px; }
        .content { margin-left:220px; padding:20px; width: calc(100% - 220px); }
        .card i { font-size:2rem; }
        .table thead { background-color:#0d6efd; color:white; }
        .table tbody tr:hover { background-color:#e9f5ff; }
    </style>
</head>
<body>

<!-- Sidebar -->
<div class="sidebar">
    <h4>Admin Panel</h4>
    <ul class="nav flex-column">
        <li><a href="AdminDashboard.jsp" class="nav-link"><i class="bi bi-speedometer2"></i> Dashboard</a></li>
        <li><a href="Employees.jsp" class="nav-link"><i class="bi bi-building"></i> Manage Employees</a></li>
        <li><a href="ViewTransactions.jsp" class="nav-link"><i class="bi bi-people"></i> View Transactions</a></li>
        <li><a href="Reports.jsp" class="nav-link active"><i class="bi bi-bar-chart"></i> Reports</a></li>
        <li><a href="Setting.jsp" class="nav-link"><i class="bi bi-gear"></i> Settings</a></li>
        <li><a href="../Login.jsp" class="nav-link text-danger"><i class="bi bi-box-arrow-right"></i> Logout</a></li>
    </ul>
</div>

<div class="content container-fluid">
    <h3 class="text-primary mb-4">Daily Bank Report (Today)</h3>

    <!-- Summary Cards -->
    <div class="row g-3 mb-4">
        <div class="col-md-2">
            <div class="card text-bg-success p-3 shadow-sm text-center">
                <i class="bi bi-arrow-down-circle"></i>
                <h6 class="mt-2">Deposits</h6>
                <p class="fs-5 fw-bold">₹ <%= dailyDeposits %></p>
            </div>
        </div>
        <div class="col-md-2">
            <div class="card text-bg-danger p-3 shadow-sm text-center">
                <i class="bi bi-arrow-up-circle"></i>
                <h6 class="mt-2">Withdrawals</h6>
                <p class="fs-5 fw-bold">₹ <%= dailyWithdrawals %></p>
            </div>
        </div>
        <div class="col-md-2">
            <div class="card text-bg-info p-3 shadow-sm text-center">
                <i class="bi bi-arrow-repeat"></i>
                <h6 class="mt-2">Transactions</h6>
                <p class="fs-5 fw-bold"><%= dailyTransactions %></p>
            </div>
        </div>
        <div class="col-md-2">
            <div class="card text-bg-warning p-3 shadow-sm text-center">
                <i class="bi bi-cash-stack"></i>
                <h6 class="mt-2">Bank Balance</h6>
                <p class="fs-5 fw-bold">₹ <%= totalBankBalance %></p>
            </div>
        </div>
        <div class="col-md-2">
            <div class="card text-bg-primary p-3 shadow-sm text-center">
                <i class="bi bi-people-fill"></i>
                <h6 class="mt-2">Active Accounts</h6>
                <p class="fs-5 fw-bold"><%= activeAccountsToday %></p>
            </div>
        </div>
    </div>

    <!-- Deposit vs Withdrawal Chart -->
    <div class="row mb-4">
        <div class="col-md-6">
            <canvas id="dailyChart"></canvas>
        </div>
    </div>

    <script>
        const ctx = document.getElementById('dailyChart').getContext('2d');
        new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ['Deposits', 'Withdrawals'],
                datasets: [{
                    label: 'Amount (₹)',
                    data: [<%= dailyDeposits %>, <%= dailyWithdrawals %>],
                    backgroundColor: ['green','red']
                }]
            },
            options: { responsive:true, plugins:{legend:{display:false}}, scales:{y:{beginAtZero:true}} }
        });
    </script>

</div>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>
